...
// cache.
// Get获取给定silence的matchers，如果cache访问失败，则它会编译并且增加matchers给定的silence到cache
...
// add编译一个sliences的matchers并且增加它们到cache中，它返回编译好的matchers
...
func (s *Silences) Set(sil *pb.Silence) (string, error) {
	...
	// 如果now在StartsAt之前，则设置StartsAt为now
...
// QMatches返回silences匹配给定的label set
...
// loadSnapshot加载由Snapshot()产生的snapshot到state，任何之前的state都会被擦除
func (s *Silences) loadSnapshot(r io.Reader) error {
	...
	for _, e := range st {
		...
		// Comments list都被移动到一个单一的comment
